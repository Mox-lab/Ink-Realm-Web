# ============================================================
# Ink Realm Web Dockerfile(多阶段构建)
# ============================================================
# 阶段:
#   1. build:  node:22-alpine + pnpm 编译产出 dist
...
# 健康检查:nginx 自身 80 端口
# 注意:必须用 127.0.0.1 而非 localhost —— BusyBox 的 wget 解析 localhost 优先走 IPv6(::1),
# 而 nginx 仅监听 IPv4(0.0.0.0:80),且 BusyBox wget 不会回退 IPv4,会导致健康检查误报 unhealthy。
